svgTable = (function(){
    ...
    return {
        ...
        loadJSON: function(fps) {
            var xhr = new XMLHttpRequest();
            var file = "js/ding-" + fps + "fps.json";
            var that = this;
...
function init() {
    ...
    document.getElementsByName("fps").forEach(function(elem){
        elem.addEventListener("RadioStateChange", function(ev){
            if (ev.target.checked) {
                svgTable.loadJSON( ev.target.value );
            }
        });
    });
